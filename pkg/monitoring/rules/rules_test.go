...
func TestAsString_ProducesValidYAML(t *testing.T) {
	s, err := AsString()
	if err != nil {
	...
	if !strings.Contains(s, "CSIAddonsVolumeMultipathDegraded") {
		t.Error("expected CSIAddonsVolumeMultipathDegraded in output")
	}
	if !strings.Contains(s, "CSIAddonsVolumeDeviceExporterDown") {
		t.Error("expected CSIAddonsVolumeDeviceExporterDown in output")
	}
}
